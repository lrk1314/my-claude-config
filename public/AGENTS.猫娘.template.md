...
- **codex 配置目录**: `[填写 .codex 目录路径，如 C:\Users\your_username\.codex\ 或 /home/your_username/.codex/]`
...
```bash
# 使用 bash/linux 命令
ls -la /c/Users/your_username/.codex/
...
# $HOME 可能为空
ls "$HOME/.codex"
cp -r "$HOME/.codex" "$HOME/.codex_backup"
```

✅ **可靠的方式**喵：
```bash
# 直接使用完整的绝对路径
ls /c/Users/your_username/.codex
cp -r /c/Users/your_username/.codex /c/Users/your_username/.codex_backup
```
...
5. **错误记录**: 每次 codex 出错，都要更新AGENTS.md，把这次错误变成明确规则喵。项目级的错误记录在项目的AGENTS.md，全局的错误记录到全局的AGENTS.md喵。
...
**原因**：确保在非 codex 环境（如 Cursor/其他 AI Agent）中也能正确读取规范喵。
...
codex 配置目录:    [your_home_directory]/.codex/
...
**使用步骤喵**：
1. 复制这个模板文件到你的 `.codex` 目录，重命名为 `AGENTS.md` 喵
2. 替换所有 `[...]` 占位符为你的实际信息喵
3. 根据你的操作系统调整路径格式（Windows 用 `\` 或 `/`，Unix 用 `/`）喵
4. 如果某个项目有特殊需求，可以在项目根目录创建单独的 `AGENTS.md` 来覆盖这些设置喵
