---
name: pptx
description: "演示文稿创建、编辑和分析工具。当 Claude 需要处理演示文稿（.pptx 文件）时使用：(1) 创建新演示文稿，(2) 修改或编辑内容，(3) 使用布局，(4) 添加注释或演讲者备注，或任何其他演示文稿任务"
...
用户可能会要求你创建、编辑或分析 .pptx 文件的内容。.pptx 文件本质上是一个包含 XML 文件和其他资源的 ZIP 归档文件，你可以读取或编辑。你有不同的工具和工作流程可用于不同的任务。
...
### 原始 XML 访问
对于以下功能，你需要原始 XML 访问：注释、演讲者备注、幻灯片布局、动画、设计元素和复杂格式。对于这些功能，你需要解包演示文稿并读取其原始 XML 内容。

#### 解包文件
`python ooxml/scripts/unpack.py <office_file> <output_dir>`

**注意**：unpack.py 脚本位于相对于项目根目录的 `skills/pptx/ooxml/scripts/unpack.py`。如果脚本不存在于此路径，请使用 `find . -name "unpack.py"` 来定位它。
...
#### 排版和颜色提取
**当给出要模仿的示例设计时**：始终先使用以下方法分析演示文稿的排版和颜色：
...
1. **考虑主题内容**：这个演示文稿是关于什么的？它建议什么样的语气、行业或情绪？
...
- **大胆尝试**：尝试意想不到的组合 - 医疗保健演示文稿不一定是绿色，金融不一定是海军蓝
...
2. **青绿和珊瑚**：青绿色 (#5EA8A7)、深青绿 (#277884)、珊瑚色 (#FE4447)、白色 (#FFFFFF)
...
6. **深紫和翡翠**：紫色 (#B165FB)、深蓝 (#181B24)、翡翠色 (#40695B)、白色 (#FFFFFF)
7. **奶油和森林绿**：奶油色 (#FFE1C7)、森林绿 (#40695B)、白色 (#FCFCFC)
8. **粉色和紫色**：粉红色 (#F8275B)、珊瑚色 (#FF574A)、玫瑰色 (#FF737D)、紫色 (#3D2F68)
9. **酸橙和李子**：酸橙色 (#C5DE82)、李子色 (#7C3A5F)、珊瑚色 (#FD8C6E)、蓝灰色 (#98ACB5)
10. **黑色和金色**：金色 (#BF9A4A)、黑色 (#000000)、奶油色 (#F4F6F6)
11. **鼠尾草和赤陶**：鼠尾草色 (#87A96B)、赤陶色 (#E07A5F)、奶油色 (#F4F1DE)、炭灰色 (#2C2C2C)
12. **炭灰和红色**：炭灰色 (#292929)、红色 (#E33737)、浅灰色 (#CCCBCB)
13. **鲜艳橙**：橙色 (#F96D00)、浅灰色 (#F2F2F2)、炭灰色 (#222831)
...
18. **橙色和青绿**：浅橙色 (#FC993E)、灰青绿色 (#667C6F)、白色 (#FCFCFC)
...
### 布局提示
**创建带有图表或表格的幻灯片时：**
- **双列布局（首选）**：使用跨越全宽的标题，然后在下方使用两列 - 一列中放置文本/要点，另一列中放置特色内容。这提供了更好的平衡，使图表/表格更易读。使用不等列宽的 flexbox（例如 40%/60% 分割）为每种内容类型优化空间。
- **全幻灯片布局**：让特色内容（图表/表格）占据整个幻灯片以获得最大影响和可读性
- **永远不要垂直堆叠**：不要在单列中将图表/表格放在文本下方 - 这会导致可读性差和布局问题

### 工作流程
1. **必须 - 读取整个文件**：从头到尾完整阅读 [`html2pptx.md`](html2pptx.md)。**永远不要在读取此文件时设置任何范围限制。** 在继续创建演示文稿之前，阅读完整的文件内容以获取详细语法、关键格式规则和最佳实践。
2. 为每张幻灯片创建一个 HTML 文件，使用适当的尺寸（例如 16:9 格式为 720pt × 405pt）
   - 对所有文本内容使用 `<p>`、`<h1>`-`<h6>`、`<ul>`、`<ol>`
   - 对将添加图表/表格的区域使用 `class="placeholder"`（用灰色背景渲染以提高可见性）
...
编辑现有 PowerPoint 演示文稿中的幻灯片时，你需要使用原始 Office Open XML (OOXML) 格式。这涉及解包 .pptx 文件、编辑 XML 内容并重新打包。

### 工作流程
1. **必须 - 读取整个文件**：从头到尾完整阅读 [`ooxml.md`](ooxml.md)（约 500 行）。**永远不要在读取此文件时设置任何范围限制。** 在进行任何演示文稿编辑之前，阅读完整的文件内容以获取有关 OOXML 结构和编辑工作流程的详细指导。
2. 解包演示文稿：`python ooxml/scripts/unpack.py <office_file> <output_dir>`
3. 编辑 XML 文件（主要是 `ppt/slides/slide{N}.xml` 和相关文件）
4. **关键**：在每次编辑后立即验证，并在继续之前修复任何验证错误：`python ooxml/scripts/validate.py <dir> --original <file>`
5. 打包最终演示文稿：`python ooxml/scripts/pack.py <input_directory> <office_file>`

## 使用模板创建新的 PowerPoint 演示文稿

当你需要创建遵循现有模板设计的演示文稿时，你需要在替换占位符上下文之前复制和重新排列模板幻灯片。

### 工作流程
1. **提取模板文本并创建视觉缩略图网格**：
   * 提取文本：`python -m markitdown template.pptx > template-content.md`
   * 读取 `template-content.md`：阅读整个文件以了解模板演示文稿的内容。**永远不要在读取此文件时设置任何范围限制。**
...
2. **分析模板并将清单保存到文件**：
   ...
   * 在 `template-inventory.md` 创建并保存模板清单文件，包含：
     ...
     # 模板清单分析
     **总幻灯片数：[数量]**
     **重要：幻灯片是 0 索引的（第一张幻灯片 = 0，最后一张幻灯片 = 数量-1）**
...
3. **基于模板清单创建演示文稿大纲**：
   ...
   * **关键：将布局结构与实际内容匹配**：
     - 单列布局：用于统一叙述或单个主题
     - 双列布局：仅在你有恰好 2 个不同的项目/概念时使用
     - 三列布局：仅在你有恰好 3 个不同的项目/概念时使用
     - 图像 + 文本布局：仅在你有实际图像要插入时使用
     - 引用布局：仅用于来自人的实际引用（带归属），永远不要用于强调
     - 永远不要使用占位符多于你拥有的内容的布局
     - 如果你有 2 个项目，不要强制它们进入 3 列布局
     - 如果你有 4+ 个项目，考虑拆分为多个幻灯片或使用列表格式
   * 在选择布局之前计算你的实际内容片段
   ...
   * 模板映射示例：
      ```
      # 要使用的模板幻灯片（基于 0 的索引）
      # 警告：验证索引是否在范围内！有 73 张幻灯片的模板索引为 0-72
      # 映射：大纲中的幻灯片编号 -> 模板幻灯片索引
      template_mapping = [
          0,   # 使用幻灯片 0（标题/封面）
          34,  # 使用幻灯片 34（B1：标题和正文）
          34,  # 再次使用幻灯片 34（第二个 B1 的副本）
          50,  # 使用幻灯片 50（E1：引用）
...
5. **使用 `inventory.py` 脚本提取所有文本**：
   * **运行清单提取**：
     ...
     ```
   * **读取 text-inventory.json**：阅读整个 text-inventory.json 文件以了解所有形状及其属性。**永远不要在读取此文件时设置任何范围限制。**
              ...
              "paragraphs": [
                {
                  ...
                  "space_before": 10.0,     // 段落前的间距（以磅为单位）
                  "space_after": 6.0,       // 段落后的间距（以磅为单位）
                  "line_spacing": 22.4,     // 行距（以磅为单位）
   ...
   * 关键特性：
     ...
     - **默认字体大小**：从布局占位符提取的 `default_font_size`（以磅为单位）（如果可用）
...
6. **生成替换文本并将数据保存到 JSON 文件**
   ...
   - **验证**：replace.py 脚本将验证替换 JSON 中的所有形状是否存在于清单中
     - 如果你引用不存在的形状，你将收到显示可用形状的错误
     - 如果你引用不存在的幻灯片，你将收到指示幻灯片不存在的错误
     - 所有验证错误在脚本退出之前会一次性显示
   ...
   - **基本格式规则**：
     ...
     - 颜色：RGB 使用 `"color": "FF0000"` 或主题颜色使用 `"theme_color": "DARK_1"`
     - 替换脚本期望**正确格式化的段落**，而不仅仅是文本字符串
     - **重叠形状**：首选具有较大 default_font_size 或更合适的 placeholder_type 的形状
   - 将更新后的清单和替换保存到 `replacement-text.json`
...
要以视觉方式分析 PowerPoint 幻灯片，使用两步过程将它们转换为图像：
...
- `-jpeg`：输出 JPEG 格式（如果首选，使用 `-png` 输出 PNG）
